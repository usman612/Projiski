...
{	
	$obj->saveGeneralCommunication($_POST['detail1'],$_POST['userid1'],$_POST['admin1']);
}
if(isset($_POST['action']))
{
	...
	foreach($result as $row)
	{
		if($row['sender']!=1)
        {
        $chat.='<p style="padding:10px;background:lightblue;">'.'<span style="font-weight:bold;border-radius:10px;">'.$row['fname'].'</span>'. ' <br/><span>' . $row['project_detail'].'</span><span style="float:right;">' . $row['date'].'</span></p>';
        }
        else
        {
             $chat.='<p style="padding:10px;background:lightgrey;">'.'<span style="font-weight:bold;border-radius:10px;">'.$row['fname'].'</span>'. ' <br/><span>' . $row['project_detail'].'</span><span style="float:right;">' . $row['date'].'</span></p>';
        }
	}
