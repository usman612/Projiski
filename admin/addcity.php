...
{
	$singleCity = $obj->getSingleCity($_GET['cityid']);
	$selectedCountry = $obj->getSelectedCountry($_GET['cityid']);
}
...
{	
	if(isset($_GET['cityid']))
	{
		...
		$update = $obj->updateCity($_GET['cityid'],$_POST['city'],$_POST['country']);

		if ($update) {
            $_SESSION['message1'] = '<b>Success!</b> City Updated successfully.';
        } else {
            $_SESSION['message1'] = '<b>Error!</b> City could not be Updated.';
        }
	}
	else{
   
	$save = $obj->saveCity($_POST['city'],$_POST['country']);
	
		if ($save===true) {
            $_SESSION['alert'] = '<b>Success!</b> City saved successfully.';
        } 
        if ($save===false) {
	...
	<div class="main-content">
		...
		<div class="row">
			<div class="col-md-12">
              ...
              $selected = '';
                ...
                if($countries[$i]['country_id'] == $selectedCountry['country_id']){
          ...
          <?php } ?>
							...
							<label for="field-1" class="col-sm-3 control-label">City</label>
								...
								<div class="col-md-3 col-sm-6">
									<input type="text" class="form-control" id="field-1" placeholder="Name" name="city" value="<?php echo isset($singleCity['city'])?$singleCity['city']:""?>" required>
								</div>
		...
		<table class="table table-striped">
			...
			<tr>
				...
				<td><a href="addcity.php?cityid=<?php echo $city['city_id'];?>"><button class="btn btn-sm btn-info">Edit<span><i class="entypo-pencil"></i></span></button></a></td>
				<td><button class="btn btn-sm btn-danger" onclick="delData(<?php echo $city['city_id'];?>)">Delete<span><i class="entypo-trash"></i></span></button></a></td>
							<!-- <td><button class="btn btn-sm btn-danger" onclick="delData(<?php echo $user['user_id'];?>)">Delete<span><i class="entypo-trash"></i></span></button></a></td> -->
			</tr>
			<?php endforeach;?>
		</table>
